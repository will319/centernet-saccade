...
from .attn import WindowAttention_conv2qkv as Attention
...
def make_attn_layer_revr(kernel_size, inp_dim, out_dim, modules, layer, window_hw):
    layers = [layer(kernel_size, inp_dim, inp_dim) for _ in range(modules - 1)]
    if window_hw == 64 or window_hw == 16:
        layers += [attn_model(dim=inp_dim, window_hw=window_hw)]
...
class kp_module(nn.Module):
    def __init__(self, n, dims, modules, window_hw=128):
        ...
        self.low3 = make_attn_layer_revr(3, next_dim, curr_dim, curr_modules,
                                         layer=residual, window_hw=window_hw // 2)
