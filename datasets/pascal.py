...
VOC_NAMES = ['__background__', 'ball', 'cylinder', 'square cage', 'cube', 'circle cage', 'human body',
             'metal bucket', 'tyre']
VOC_NAMES_ = ['ball         ',
              'cylinder     ',
              'square cage  ',
...
class PascalVOC(data.Dataset):
    def __init__(self, data_dir, split, split_ratio=1, img_size=512,
                 img_dir='./imgs_path/', *args, **kwargs):
        super(PascalVOC, self).__init__()
        self.num_classes = 8  # class num(not the background in it)
        ...
        _ann_name = {'train': 'test', 'val': 'val'}
        self.annot_path = os.path.join(self.data_dir, 'annotations', '%s.json' % _ann_name[split])
    ...
    def __getitem__(self, index):
        ...
        center = np.array([width / 2., height / 2.], dtype=np.float32)  # center of images
        ...
        trans_img = get_affine_transform(center, scale, 0,
        ...
        trans_fmap = get_affine_transform(center, scale, 0,
        ...
        for k, (bbox, label) in enumerate(zip(bboxes, labels)):
            ...
            if h > 0 and w > 0:
                ...
                # cv.imshow('hmap', hmap[label])    # show the heatmap
                # cv.waitKey(0)
                w_h_[k] = 1. * w, 1. * h
...
class PascalVOC_eval(PascalVOC):
    def __init__(self, data_dir, split, test_scales=(1,), test_flip=False, fix_size=True, *args, **kwargs):
        super(PascalVOC_eval, self).__init__(data_dir, split, *args, **kwargs)  # test_scale = [0.5,0.75,1,1.25,1.5]
    ...
    def run_eval(self, results, save_dir=None):
        ...
        if save_dir is not None:
            torch.save(detections, os.path.join(save_dir, 'checkpoint.t7'))
        eval_map = eval_mAP()
...
class eval_mAP:
    def __init__(self, YEAR='x1000', set='test'):
        self.VOC_root = '/'
        self.YEAR = YEAR
        self.set_type = set
        self.annopath = os.path.join(self.VOC_root, 'VOCx1000', 'Annotations', '{:s}.xml')
        self.imgsetpath = os.path.join(self.VOC_root, 'VOCx1000', 'ImageSets', 'Main', '%s.txt')
        self.devkit_path = os.path.join(self.VOC_root, 'VOC' + YEAR)
    ...
    def parse_record(self, filename):
        ...
        for obj in tree.findall('object'):
            ...
            obj_struct['pose'] = 'Unknow'  # obj.find('pose').text
    ...
    def do_python_eval(self, detections, use_07=False):
        ...
        for i, cls in enumerate(VOC_NAMES[1:]):
            rec, prec, ap = self.voc_eval(detections[cls], self.annopath,
                                          self.imgsetpath % self.set_type,  # test
                                          cls, cachedir, ovthresh=0.7, use_07_metric=use_07)  # ovthresh=0.5
            aps += [ap]
            # print('AP for %s = %.2f%%' % (cls, ap * 100))
            print('AP for %s = %.2f%%, recall = %.2f%%, precision = %.2f%%' % (
            VOC_NAMES_[i], ap * 100, np.average(rec) * 100, np.average(prec) * 100))
...
if __name__ == '__main__':
    import tqdm
    val_dataset = PascalVOC_eval(r'./data', 'val',
                                 img_dir='/')
    ...
    for d in tqdm.tqdm(val_dataset):
        pass
